...
export default defineNuxtConfig({
  ...
  app: {
    head: {
      script: [
        {
          // 首屏内联脚本：在首次绘制前从 localStorage 恢复主题、字号与字体，
          // 避免刷新时先亮后暗、先小后大、先默认字体再换字体的闪烁
          // （hydration 后由 useTheme / useFontScale / useFontFamily 接管）。
          // 注意：这里的 storage key 必须与三个 composable 保持一致。
          innerHTML: [
            '(function(){try{',
